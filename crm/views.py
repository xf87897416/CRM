    ...
    if request.method == "POST":
        ...
        if enroll_form.is_valid(): #验证成功
            ...
            try:
                ...
                enroll_form.cleaned_data["customer"] = customer_obj   #有个字段不在form验证，会报错 #强行塞进去
                enroll_obj = models.Enrollment.objects.create(**enroll_form.cleaned_data)   #创建数据
...
def testlogin(request):
    if request.method == "POST":
        if request.is_ajax():
            enroll_data_dir = "%s" % (settings.ENROLLED_DATA)
            if not os.path.exists(enroll_data_dir):
                os.makedirs(enroll_data_dir, exist_ok=True)
            ...
            #     dirt['name'] = name
            #     dirt['age'] = age
            # return HttpResponse(json.dumps(dirt))


    return render(request,"king_admin/testlogin.html")
