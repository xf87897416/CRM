...
class StudyRecord(models.Model):
    ...
    course_record = models.ForeignKey("CourseRecord") #选择课程
    attendance_choices = (
        (0, '已签到'),
        (1, '迟到'),
        (2, '缺勤'),
...
class Enrollment(models.Model):
    ...
    contract_approved = models.BooleanField(default=False,verbose_name="合同已经核实")
    date =models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return "%s %s"%(self.customer,self.enrolled_class)  #返回学生对象，班级对象
...
class Menu(models.Model):
    ...
    url_type_choices = ((0,'alias'),(1,'absoulte_url'))
